...
def render_bar(panel, x, y, total_width, name, value, maximum, bar_color, back_color):
    ...
    tcod.console_print_ex(panel, int(x + total_width / 2), y,
                          tcod.BKGND_NONE, tcod.CENTER, f'{name}: {value}/{maximum}')
...
def render_all(con, panel, entities, player, game_map, fov_map,
                ...
                if visible:
                    if wall:
                        tcod.console_set_char_background(
                            con, x, y, colors.get(
                                "light_wall"), tcod.BKGND_SET
                        )
                    else:
                        tcod.console_set_char_background(
                            con, x, y, colors.get(
                                "light_ground"), tcod.BKGND_SET
                        )
                    ...
                    if wall:
                        tcod.console_set_char_background(
                            con, x, y, colors.get(
                                "dark_wall"), tcod.BKGND_SET
                        )
                    else:
                        tcod.console_set_char_background(
                            con, x, y, colors.get(
                                "dark_ground"), tcod.BKGND_SET
                        )
    ...
    render_bar(panel, 1, 1, bar_width, 'HP', player.fighter.hp,
               player.fighter.max_hp, tcod.lighter_red, tcod.darker_red)
    tcod.console_print_ex(panel, 1, 3, tcod.BKGND_NONE,
                          tcod.LEFT, f'Dungeon level: {game_map.dungeon_level}')

    tcod.console_set_default_foreground(panel, tcod.light_gray)
    tcod.console_print_ex(
        panel, 1, 0, tcod.BKGND_NONE, tcod.LEFT, get_names_under_mouse(mouse, entities, fov_map))
